---
title: Install evlog
description: Install evlog in your TypeScript project.
navigation:
  title: Installation
  icon: i-lucide-download
links:
  - label: Frameworks
    icon: i-lucide-layout-grid
    to: /frameworks/overview
    color: neutral
    variant: subtle
---

evlog supports Nuxt, Next.js, SvelteKit, Nitro, NestJS, and any TypeScript server framework.

## Install

::code-group
```bash [pnpm]
pnpm add evlog
```
```bash [npm]
npm install evlog
```
```bash [yarn]
yarn add evlog
```
```bash [bun]
bun add evlog
```
::

### Using Agent Skills

If you use an AI assistant (Claude Code, Cursor, etc.), install the evlog skill for guided setup and code review:

```bash [Terminal]
npx skills add hugorcd/evlog
```

Your AI assistant can then help you set up evlog, review your logging patterns, and migrate existing code to wide events. See [Agent Skills](/getting-started/agent-skills) for details.

## Choose Your Framework

After installing the package, follow the setup guide for your framework:

::card-group
  :::card
  ---
  icon: i-simple-icons-nuxtdotjs
  title: Nuxt
  to: /frameworks/nuxt
  color: neutral
  ---
  Module with auto-imported `useLogger`, `createError`, and `parseError`.
  :::
  :::card
  ---
  icon: i-simple-icons-nextdotjs
  title: Next.js
  to: /frameworks/nextjs
  color: neutral
  ---
  `createEvlog()` factory with `withEvlog()` handler wrapper.
  :::
  :::card
  ---
  icon: i-simple-icons-svelte
  title: SvelteKit
  to: /frameworks/sveltekit
  color: neutral
  ---
  Handle and handleError hooks with `event.locals.log`.
  :::
  :::card
  ---
  icon: i-custom-nitro
  title: Nitro
  to: /frameworks/nitro
  color: neutral
  ---
  Module for Nitro v2 and v3 with plugin-based hooks.
  :::
  :::card
  ---
  icon: i-custom-tanstack
  title: TanStack Start
  to: /frameworks/tanstack-start
  color: neutral
  ---
  Uses Nitro v3 module with async context.
  :::
  :::card
  ---
  icon: i-simple-icons-nestjs
  title: NestJS
  to: /frameworks/nestjs
  color: neutral
  ---
  `EvlogModule.forRoot()` with global middleware.
  :::
  :::card
  ---
  icon: i-simple-icons-express
  title: Express
  to: /frameworks/express
  color: neutral
  ---
  Middleware with `req.log`.
  :::
  :::card
  ---
  icon: i-simple-icons-hono
  title: Hono
  to: /frameworks/hono
  color: neutral
  ---
  Middleware with `c.get('log')`.
  :::
  :::card
  ---
  icon: i-simple-icons-fastify
  title: Fastify
  to: /frameworks/fastify
  color: neutral
  ---
  Plugin with `request.log`.
  :::
  :::card
  ---
  icon: i-custom-elysia
  title: Elysia
  to: /frameworks/elysia
  color: neutral
  ---
  Plugin with `log` in route context.
  :::
  :::card
  ---
  icon: i-simple-icons-cloudflare
  title: Cloudflare Workers
  to: /frameworks/cloudflare-workers
  color: neutral
  ---
  Factory for request-scoped loggers.
  :::
::

::callout{icon="i-lucide-arrow-right" color="neutral"}
See the full [Framework Integrations](/frameworks/overview) page for a comparison table and all available integrations including [Standalone TypeScript](/frameworks/standalone) and [Astro](/frameworks/astro).
::

## TypeScript Configuration

evlog ships with full TypeScript type definitions. No additional configuration is required.

::callout{icon="i-lucide-check" color="success"}
evlog requires TypeScript 5.0 or higher for optimal type inference.
::

## Next Steps

- [Quick Start](/getting-started/quick-start) — Learn the core concepts and start using evlog
- [Wide Events](/core-concepts/wide-events) — Understand the wide event pattern
- [Adapters](/adapters/overview) — Send logs to Axiom, PostHog, Sentry, and more
